	...
	internal class PConsoleTableControl : PConsoleControlBase
	{
		...
		*/
		...
		public void PrintHeader()
		{
			...
			this.PrintRow("Country", "MNO", "OneMinConv", "OneMinPyt", "TwoMinConv", "ToMinPyt", "Vertical", "OS", "TConversions", "TPayout", "LConv");
			this.PrintRowSeparator();
		...
		public void PrintTableData(List<LinkDisplay> list)
		{
			foreach (Links.LinkDisplay display in list)
				PrintRow(display.Country, display.MobileOperator, display.OneMinuteConversions, display.OneMinuteAvaragePayout, display.TwoMinuteConversions, display.TwoMinuteAvaragePayout, display.Vertical,
					display.OS, display.TotalConversions, display.TotalPayout, display.LastConversions);
			this.PrintRowSeparator();
			this.WriteLine();
		...
		private void PrintRowSeparator()
		{
			...
			result += "===" + this.TrimString(separator, TableValues.TwoMinConv);
			result += "===" + this.TrimString(separator, TableValues.TwoMinPyt);
		...
		private void PrintRow(string country, string mobileOperator, string oneMinConv, string oneMinPyt, string twoMinConv, string twoMinPyt, string vertical, string os, string totalConversions, string totalPayout,  string lastConversion)
		{
			string result = "";
			...
			result += " | " + this.TrimString(twoMinConv, TableValues.TwoMinConv);
			result += " | " + this.TrimString(twoMinPyt, TableValues.TwoMinPyt);
			...
			try
			{
				...
				foreach(PushyCash.Campaigns.PushyCampaign camp in PushyContext.CampaignManager.ActiveCampaigns)
				{
					...
					foreach (var ts in camp.PushyNetworkCampaigns)
					{
						...
						else if (ts.Value.IsPaused)			Console.BackgroundColor = ConsoleColor.DarkMagenta;
						...
						this.PrintCampaignInformation("Convs", ts.Value.Conversions.ToString());
						...
						int maxNumber = PushyContext.CampaignManager.ActiveCampaigns.Count >= 5 ? 3 : ts.Value.Logs.Count;
