...
namespace PushyCash.Links
{
	public class LinkOffersConversion
	{
		...
		{
			int oneMinuteConversions = 0, twoMinuteConversions = 0;
			double oneMinutePayout = 0.0, twoMinutePayout = 0.0;
			...
			lock (lock_obj)
			{
				foreach (KeyValuePair<string, List<LinkConversion>> conv in this.Conversions)
				{
					for (int i = conv.Value.Count - 1; i >= 0; i--)
						if ((DateTime.Now - conv.Value.ElementAt(i).Created).TotalMinutes >= 2)
							conv.Value.RemoveAt(i);
						else if ((DateTime.Now - conv.Value.ElementAt(i).Created).TotalMinutes <= 1)
						{
