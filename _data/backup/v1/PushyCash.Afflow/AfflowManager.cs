...
namespace PushyCash.Afflow
{
	public class AfflowManager
	{
		...
		public List<AfflowLeagueStats> GetLeagueStatsList(string nid, DateTime? startDate = null, DateTime? endDate = null)
		{
			List<AfflowLeagueStats> result = new List<AfflowLeagueStats>();
			foreach (var entry in this.GetLeagueStats(nid, startDate, endDate)) result.Add(entry);
			return result;
		}
		public IEnumerable<AfflowLeagueStats> GetLeagueStats(string nid, DateTime? startDate = null, DateTime? endDate = null)
		{
		...
		{
			if (!startDate.HasValue) startDate = DateTime.Now.AddDays(-1);
		...
		public IEnumerable<AfflowCampaignMobileOperatorOfferMap> GetCampaignMobileOperatorMap(int nid, string campaignID) { return this._api.GetCampaignMobileOperatorMap(nid, campaignID); }
