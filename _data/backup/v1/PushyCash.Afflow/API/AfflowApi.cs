...
namespace PushyCash.Afflow.API
{
	internal class AfflowApi : AfflowApiBase
	{
		...
		public void PreformLogin()
		{
			...
			dynamic json =  this.Post(url, "&email=nsp@monkeyhou.se&pass=hpn28Y=r-!");
		...
		{
			string url = string.Format("https://api.monetizer.co/data/report.php?nid={0}&start_ts={1}&end_ts={2}", nid, this.DateToUnix(startDate), this.DateToUnix(endDate));
			dynamic json = this.Request(url);
			if (json == null || json.ToString().Equals("[]"))
				yield break;
